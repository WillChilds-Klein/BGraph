...
public class HungarianAlgorithmEdu {
	...
	static double [][] originalCost;
	static double[] oMinVals;
	static double oMax;
	...
	public static int hg_step1(int step, double[][] cost)
	{
		...
		oMinVals = new double[cost.length];
	...
	{
		//What STEP 5 does:	
		//Step 5 basically is a version of an augmenting path algorithm (for solving the
		...
		//something will be equal to. It changes every time. Therefore, we can either have the
	...
	(double[][] cost, int[] rowCover, int[] colCover, double maxCost)
	{
		...
		//System.out.println("hopefully delta = " + (oMax - oMinVals[x] + minval) + " " + x + " " + y);
		
		return minval;
	}
	
	public static double findLargest		//Aux 1 for hg_step6. ADDED BY WCK
	(double[][] cost, int[] rowCover, int[] colCover, double maxCost)
	{
